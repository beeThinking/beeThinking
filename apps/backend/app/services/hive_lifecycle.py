...
def archive_hive(db: Session, hive_id: int, owner_id: int, reason: str, event_date: date, note: str | None = None):
    hive = _get_hive(db, hive_id, owner_id)
    if not hive:
        return None
    ...
    _close_open_tasks(db, owner_id, hive_id)
    db.commit()
    db.refresh(hive)
    return hive
...
def merge_hives(db: Session, source_hive_id: int, target_hive_id: int, owner_id: int, event_date: date, note: str | None = None):
    source = _get_hive(db, source_hive_id, owner_id)
    target = _get_hive(db, target_hive_id, owner_id)
    ...
    _close_open_tasks(db, owner_id, source.id)
    db.commit()
    db.refresh(source)
    return source
...
def can_hard_delete_hive(db: Session, hive_id: int, owner_id: int) -> bool:
    ...
    dependency_count = (
        ...
        + db.query(Treatment).filter(Treatment.hive_id == hive_id, Treatment.owner_id == owner_id).count()
        + db.query(Harvest).filter(Harvest.hive_id == hive_id, Harvest.owner_id == owner_id).count()
        + db.query(Photo).filter(Photo.hive_id == hive_id, Photo.owner_id == owner_id).count()
    )
...
def get_hive_timeline(db: Session, hive_id: int, owner_id: int) -> list[HiveEvent]:
    return (
        db.query(HiveEvent)
        .filter(HiveEvent.user_id == owner_id, HiveEvent.hive_id == hive_id)
        .order_by(HiveEvent.event_date.desc(), HiveEvent.created_at.desc())
        .all()
    )


def _get_hive(db: Session, hive_id: int, owner_id: int):
    return db.query(Hive).filter(Hive.id == hive_id, Hive.owner_id == owner_id).first()


def _close_open_tasks(db: Session, owner_id: int, hive_id: int) -> None:
    db.query(Task).filter(Task.owner_id == owner_id, Task.hive_id == hive_id, Task.status == TaskStatus.open).update(
